                ...
                for block in pymu_raw_blocks:
                    ...
                    if bbox[0]-1 <= imgbox[0] < bbox[2]+1 and bbox[1]-1 <= imgbox[1] < bbox[3]+1:  # 确定在block内
                        for l in block['lines']:
                            line_box = l['bbox']
                            # 在line内的，插入line前面
                            if line_box[0]-1 <= imgbox[0] < line_box[2]+1 and line_box[1]-1 <= imgbox[1] < line_box[3]+1:
                                line_txt = "".join([s['text']
                                                   for s in l['spans']])
                            ...
                            for l in block['lines']:
                                line_box = l['bbox']
                                distance = math.sqrt(
                                    (line_box[0] - imgbox[0])**2 + (line_box[1] - imgbox[1])**2)
                                if distance < min_distance:
                                    min_distance = distance
                                    min_line = l
                            if min_line:
                                line_txt = "".join([s['text']
                                                   for s in min_line['spans']])
                    ...
                    top_txt_block = find_top_nearest_text_bbox(
...
def insert_img_or_table(type, element, pymu_raw_blocks, content_lst):
    ...
    for block in pymu_raw_blocks:
        ...
        if bbox[0] - 1 <= element_bbox[0] < bbox[2] + 1 and bbox[1] - 1 <= element_bbox[1] < bbox[
            ...
            for l in block['lines']:
                ...
                for l in block['lines']:
                    line_box = l['bbox']
                    distance = math.sqrt(
                        (line_box[0] - element_bbox[0]) ** 2 + (line_box[1] - element_bbox[1]) ** 2)
                    if distance < min_distance:
                        min_distance = distance
                        min_line = l
                if min_line:
                    ...
                    if min_distance < img_h:  # 文字在图片前面
                        __insert_after_para(
                            line_txt, type, element, content_lst)
                    else:
        ...
        if top_txt_block:
            line_txt = "".join([s['text']
                               for s in top_txt_block['lines'][-1]['spans']])
